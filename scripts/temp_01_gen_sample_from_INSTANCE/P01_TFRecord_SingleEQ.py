...
os.makedirs(outdir_single)
os.makedirs(figdir_single)
...
for j in range(len(meta_df)):
    ...
    wf = read(
        os.path.join(
            wfdir, str(info[0]), f"{str(info[0])}.{info[1]}.{info[2]}.*.{info[4]}?.sac"
        )
    )
    ...
    trc_tp = gen_tar_func(wf_npts, slice_tp_npts, int(np.round(err_win_p / 0.01)))
    trc_ts = gen_tar_func(wf_npts, slice_ts_npts, int(np.round(err_win_s / 0.01)))
    ...
    if np.logical_or(np.isinf(trc_3C).any(), np.isnan(trc_3C).any()):
        raise ValueError
    ...
    if gen_ct <= 30:
        ...
        ax[1].set_ylabel("N")
        ax[2].set_ylabel("Z")
        ax[3].plot(trc_tp, linewidth=1)
        ax[3].set_ylim(0, 1.1)
        ax[3].axvline(slice_tp_npts, linewidth=1, linestyle=":")
        ax[4].axvline(slice_ts_npts, linewidth=1, linestyle=":")
        ax[4].plot(trc_ts, linewidth=1)
        ax[4].set_ylim(0, 1.1)
        ax[5].plot(trc_mask, linewidth=1)
        ax[5].set_ylim(0, 1.1)
        ax[3].set_ylabel("P")
        ax[4].set_ylabel("S")
        ax[5].set_ylabel("mask")
    ...
    write_TFRecord_detect(trc_3C, label_psn, mask, idx=idx, outfile=outfile)
    gen_ct += 1
