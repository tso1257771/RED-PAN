...
for e in range(len(ev_num)):
    ...
    while ct <= ct_max:
        for i in range(len(machine_types)):
            ...
            read_path_info = np.array(
                [
                    os.path.join(
                        wfdir, str(u[0]), f"{u[0]}.{u[1]}.{u[2]}.*.{u[4]}?.sac"
                    )
                    for u in use_info
                ]
            )
            ...
            (
                ...
                read_path_info=read_path_info,
                psres_multiple=psres_multiple,
                base_wf_sec=base_wf_sec,
                buffer_secs_bef_P=buffer_secs_bef_P,
                dt=dt,
            )
            # stop
            if base_period == None:
            ...
            if not np.array_equal(
                np.array([len(i) for i in trc_pairs]), np.full((len(trc_pairs)), 3)
            ):
                continue

            trc_mosaic_pair, tp_to_ori_pt_pair, ts_to_ori_pt_pair = mosaic_wf_marching(
                base_wf_sec=base_wf_sec,
                dt=dt,
            ...
            for sp in range(len(trc_mosaic_pair)):
                ...
                valid_tp_to_ori_pt = tp_to_ori_pt_pair[sp][
                    np.logical_and(
                        tp_to_ori_pt_pair[sp] > 0, tp_to_ori_pt_pair[sp] < data_npts
                    )
                ]

                valid_ts_to_ori_pt = ts_to_ori_pt_pair[sp][
                    np.logical_and(
                        ts_to_ori_pt_pair[sp] > 0, ts_to_ori_pt_pair[sp] < data_npts
                    )
                ]
                ...
                outid = f"MMWA_{ev_num[e]}evs_{machine_type}.{ct+1:07}"
                outfile = os.path.join(outdir, f"{outid}.tfrecord")
                logging.info(
                    f"Generating mosaic waveform ({ev_num[e]}evs) - "
                    f"{ct}/{ct_max} \n {outfile}"
                )
                write_TFRecord_detect(
                    trc_3C, label_psn, mask, idx=outid, outfile=outfile
                )
                # write arrivals information
                tp_secs = list(np.round(valid_tp_to_ori_pt * dt, 2))
                ts_secs = list(np.round(valid_ts_to_ori_pt * dt, 2))
                out_info = (
                    f"{outfile};{tp_secs};{ts_secs}"
