...
sel_meta_idx = (
    eq_meta.station_network_code
    + "."
    + eq_meta.station_code
    + ".."
    + eq_meta.station_channels
    == sel_machine_id
)
...
pick_args = {"detection_threshold": 0.5, "P_threshold": 0.3, "S_threshold": 0.3}
### initialize continuous data processing framework
RP_picker = PhasePicker(
    model=model, pred_npts=pred_npts, dt=0.01, postprocess_config=None
)
...
for i in range(len(sel_eq_meta)):
    logging.info(
        f"Generateing sac files with REDPAN predictions: " + f"{i+1}/{len(sel_eq_meta)}"
    )
    ...
    tr_chn = ["E", "N", "Z"]
    for j in range(3):
        fake_array = np.hstack([np.zeros(pred_npts), eq_array[j], np.zeros(pred_npts)])
    ...
    _st = deepcopy(stream).detrend("demean").filter("bandpass", freqmin=1, freqmax=45)
    ...
    for _sac in stream:
        ...
        out_sac_temp = os.path.join(outsac_dir, f"{wf_idx}{chn[-1]}.sac")
    ...
    info = (
        f"{str(source_id)}\t{sta_network}\t{sta}\t{str(sta_loc_code)}\t"
