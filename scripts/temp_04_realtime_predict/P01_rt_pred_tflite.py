...
n_slices = int(np.ceil(
    (len(wf[0].data)-pred_npts)/(pred_interval_sec/dt)))
...
for i in range(n_slices):
    ...
    if init_pt+pred_npts > sWF[0].stats.npts:
        ...
        wf_E = np.insert(wf_E, len(wf_E), 
            np.zeros(pred_npts-len(wf_E)))
        wf_N = np.insert(wf_N, len(wf_N), 
            np.zeros(pred_npts-len(wf_N)))
        wf_Z = np.insert(wf_Z, len(wf_Z), 
            np.zeros(pred_npts-len(wf_Z)))
    else:
    ...
    if init_pt < buffer_npts:
        z_trc = deepcopy( 
            sWF[2].data[init_pt:init_pt+pred_npts+buffer_npts])
        buffer_npts = 0
    elif init_pt >= buffer_npts and \
        init_pt+pred_npts+buffer_npts <= sWF[0].stats.npts:
        z_trc = deepcopy( sWF[2].data[
            init_pt-buffer_npts:init_pt+pred_npts+buffer_npts])
    else:
        z_trc = deepcopy( sWF[2].data[
            init_pt-buffer_npts:init_pt+pred_npts])
    ...
    model.set_tensor(input_details[0]['index'],np.stack(
        [wf_E, wf_N, wf_Z], -1)[np.newaxis, ...].astype('float32'))
    model.invoke()
    ...
    for _p in p_peaks:
        ...
        check_trgP = len(np.where(np.array(
            [_p in range(s[0], s[1]) for s in search_range]))[0])
    ...
    if len(p_peaks) < 1:
        ...
        # estimate local SNR
        p_snrs, max_signal_amp = snr_joints(z_trc, 
            jt_pt=p_peaks+buffer_npts, snr_win=1,
            dt=dt, mode='std', bandpass=True, freqmin=1, freqmax=45)
        ...
        for t in range(len(p_peaks)):
            ...
            #  3 seconds error
            check_issued = len(np.where(np.abs(
                (p_peaks[t]+init_pt) - np.array(trg_pool)) < \
                (real_time_p_trg_search[1]-real_time_p_trg_search[0]))[0]
                ) > 0
    ...
    pred_array_P[:pred_npts-pred_interval_pt] = \
        pred_array_P[pred_interval_pt:]
    pred_array_P[pred_npts-pred_interval_pt:] = \
        [[] for _ in range(pred_interval_pt)]
    pred_array_S[:pred_npts-pred_interval_pt] = \
        pred_array_S[pred_interval_pt:]
    pred_array_S[pred_npts-pred_interval_pt:] = \
        [[] for _ in range(pred_interval_pt)]
    pred_array_mask[:pred_npts-pred_interval_pt] = \
        pred_array_mask[pred_interval_pt:]
    pred_array_mask[pred_npts-pred_interval_pt:] = \
        [[] for _ in range(pred_interval_pt)]
    ...
    try:
        ...
        if plt_fig :#and i == n_slices-1:
            ...
            _P, _S, _M = np.zeros( wf[0].stats.npts), \
                np.zeros( wf[0].stats.npts), np.zeros( wf[0].stats.npts)
            ...
            ax[1].axvspan(init_pt*dt, (init_pt+pred_interval_pt)*dt,
                color='b', alpha=0.2, label='renewed PDF', linewidth=1)
            ax[1].legend()
            ...
            for r in range(2, 5):
                ...
                ax[r].axvline((init_pt+pred_npts)*dt, color='r', 
                    label='current time', linewidth=1)
                ax[r].axvspan(init_pt*dt, (init_pt+pred_npts)*dt, 
                    color='r', alpha=0.1, label='prediction frame')
...
if len(trg_pool) != 0:
    for ts in range(len(trg_pool)):
        ...
        trg_info = f"{info.network}, {info.station}, "+\
            f"{trg_frame_utc}, "+\
