...
# # to call inference
# fake_input = np.random.random([5, 3, 3000]).astype('float32') #Z,N,E order
# out_vector = model(torch.from_numpy(fake_input))
...
def conti_standard_wf_fast(wf, pred_npts, pred_interval_sec, dt, pad_zeros=True):
    ...
    wf = wf.sort()[::-1] # in Z-N-E order
    wf = sac_len_complement(deepcopy(wf), max_length=full_len)
...
class sbm_PhasePicker:
    def __init__(
        ...
        if sbm_model == None:
            AssertionError("The Seisbench model should be defined!")

    def predict(self, wf=None, postprocess=False, model_name=None):
        ...
        if self.model.name.lower() in ['eqtransformer']:
            tensor_mask, tensor_p, tensor_s = self.model(
                torch.from_numpy(wf_slices.astype('float32')))
            trc_mask = tensor_mask.detach().numpy()
            trc_p = tensor_p.detach().numpy()
            trc_s = tensor_s.detach().numpy()
            # Defined for pred_MedianFilter
            masks = trc_mask[..., np.newaxis]
            predPhase = np.stack([trc_p, trc_s], axis=-1)

        elif self.model.name.lower() in ['phasenet']:
            #predPhase, masks = self.model.predict(wf_slices)
            out_vector = self.model(
                torch.from_numpy(wf_slices.astype('float32'))).detach().numpy()
            # change axis to [0, 2, 1]
            out_vector = np.transpose(out_vector, [0, 2, 1])
            nz = out_vector[..., 0]
            # Defined for pred_MedianFilter
            masks = None
            predPhase = out_vector[..., 1:3]

        else:
            raise ValueError(
                "The Seisbench model name should be defined in ['EQTransformer', 'PhaseNet']!")
        ...
        if not (masks is None):
            ...
            return array_P_med, array_S_med
    ...
    def annotate_stream(self, wf, STMF_max_sec=None, postprocess=False):
        ...
        if wf[0].stats.npts <= self.pred_npts:
            ...
            # sort to Z-N-E manner
            _wf.sort()
            _wf = _wf[::-1]

            if self.model.name.lower() in ['eqtransformer']:
                P_stream, S_stream, M_stream = Stream(), Stream(), Stream()
                in_data = np.array([W.data.astype('float32') for W in _wf])[np.newaxis, ...]
                tensor_mask, tensor_p, tensor_s = self.model(
                    torch.from_numpy(in_data))
                array_m = tensor_mask.detach().numpy()[0]
                array_p = tensor_p.detach().numpy()[0]
                array_s = tensor_s.detach().numpy()[0]
                ...
                W_chn = ["EQTransformer_P", "EQTransformer_S", "EQTransformer_mask"]
                ...
                wf.append(P_stream[0])
                wf.append(S_stream[0])
                wf.append(M_stream[0])
                
            elif self.model.name.lower() in ['phasenet']:
                P_stream, S_stream = Stream(), Stream()
                in_data = np.array([W.data.astype('float32') for W in _wf])[np.newaxis, ...]
                out_vector = self.model(torch.from_numpy(in_data))

                # change axis to [0, 2, 1]
                out_vector = np.transpose(out_vector.detach().numpy(), [0, 2, 1])
                array_p = out_vector[0].T[1]
                array_s = out_vector[0].T[2]

                gc.collect()
                # replace nan by 0
                find_nan = np.where(np.isnan(array_p))[0]
                array_p[find_nan] = np.zeros(len(find_nan))
                array_s[find_nan] = np.zeros(len(find_nan))
                # replace inf by 0
                find_inf = np.where(np.isnan(array_p))[0]
                array_p[find_inf] = np.zeros(len(find_inf))
                array_s[find_inf] = np.zeros(len(find_inf))

                W_data = [array_p, array_s]
                W_chn = ["PhaseNet_P", "PhaseNet_S"]
                W_sac = [P_stream, S_stream]
                for k in range(2):
                    W = deepcopy(_wf[0])
                    W.data = W_data[k]
                    W.stats.channel = W_chn[k]
        ...
        # STMF_max_sec/delta
            ...
            ## append to waveform and return
            if self.model.name.lower() in ['eqtransformer']:
                W_data = [array_P_med, array_S_med, array_M_med]
                W_chn = ["EQTransformer_P", "EQTransformer_S", "EQTransformer_Detection"]
                W_sac = [P_stream, S_stream, M_stream]
                for k in range(3):
                    W = deepcopy(_wf[0])
                    W.data = W_data[k]
                ...
                W_data = [array_P_med, array_S_med]
                W_chn = ["PhaseNet_P", "PhaseNet_S"]
                W_sac = [P_stream, S_stream]
                for k in range(2):
                    W = deepcopy(_wf[0])
        ...
        if self.model.name.lower() in ['eqtransformer']:
            ...
            P_stream, S_stream = Stream(), Stream()
            for S in range(seg_n):
                _P_stream, _S_stream = Stream(), Stream()
                if S == 0:
                    seg_slice_stt = seg_wf_stt[S]
                else:
                    seg_slice_stt = seg_wf_stt[S] - self.pred_npts*self.dt
