...
class REDPAN:
    ...
    def _create_gaussian_weights(self) -> np.ndarray:
        """
        Create uniform position weights (replaces Gaussian to fix time shift bug)
        
        The original Gaussian weighting caused time shift issues because early
        windows contain mostly padding, but Gaussian weights are centered at 
        window midpoint. This creates misalignment between predictions and data.
        
        Uniform weights match the legacy median filter behavior and ensure
        consistent time alignment across all window positions.
        
        Returns:
            Uniform weights for all positions in prediction window
        """
        # Use uniform weights (all ones) instead of Gaussian
        # This matches the legacy pred_MedianFilter behavior
        weights = np.ones(self.pred_npts, dtype=np.float32)
        
        logger.debug(f"Created uniform weights with shape {weights.shape}")
        return weights
    ...
    def _seisbench_accumulation(self, 
        ...
        CRITICAL FIX: Account for padding offset when extracting predictions.
        Each window was extracted from padded data starting at (pred_interval_pt * i),
        so window 0 starts at position 0 in padded space.
        The accumulation must use the same coordinate system.
        """
        ...
        logger.debug(f"Padding: before={pad_before}, after={pad_after}")
        
        # Process each prediction slice with DIRECT accumulation
        for i in range(len(predictions)):
            # Calculate position for this prediction IN PADDED SPACE
            # Windows were extracted starting at (pred_interval_pt * i) from padded array,
            # so accumulation must use the same positions
            start_pos = i * self.pred_interval_pt
            end_pos = start_pos + self.pred_npts
            
            # FIX: Only accumulate the part of predictions that corresponds to REAL data
            # Real data occupies positions [pad_before : pad_before + wf_npts] in padded space
            data_start = pad_before
            data_end = pad_before + wf_npts
            
            # Calculate overlap between this window and the real data region
            overlap_start = max(start_pos, data_start)
            overlap_end = min(end_pos, data_end)
            
            if overlap_start >= overlap_end:
                # This window doesn't overlap with real data at all (pure padding)
                continue
            
            # Calculate which part of the prediction corresponds to real data
            pred_start_idx = overlap_start - start_pos  # Index within the prediction array
            pred_end_idx = overlap_end - start_pos
            
            # Extract only the part of predictions that corresponds to real data
            pred = predictions[i][pred_start_idx:pred_end_idx]
            mask = masks[i][pred_start_idx:pred_end_idx]
            weights = self.position_weights[pred_start_idx:pred_end_idx]
            ...
            # DIRECT ACCUMULATION - only for the overlap region
            P_accumulator[overlap_start:overlap_end] += pp * weights
            S_accumulator[overlap_start:overlap_end] += ss * weights
            M_accumulator[overlap_start:overlap_end] += mm * weights
            weight_accumulator[overlap_start:overlap_end] += weights
