...
class unets:
    ...
    def mtan_att_block(self, pre_att_layer, pre_target, target, 
            nb_filter_in, nb_filter_out, strides,
            mode=None, name=None):
        ## used in decoder, apply up-sampling
        if mode == 'up':
            x = self.upconv_unit(pre_att_layer, 
                nb_filter=nb_filter_out, concatenate_layer=pre_target)
        ...
        if mode == 'down':
            ...
            mul = self.conv_unit(mul, 
                nb_filter=nb_filter_out, stride_size=strides, 
                name=name)    
        return mul
    ...
    def att_block(self, xl, gate, name=None):
        ...
        W_x = Conv1D(F_l, F_int, strides=1, padding=self.padding,
                bias_regularizer=self.bias_regu,
                kernel_initializer=self.kernel_init, 
                kernel_regularizer=self.kernel_regu)(xl)
        W_x_n = BatchNormalization()(W_x)

        W_g = Conv1D(F_g, F_int, strides=1, padding=self.padding,
                bias_regularizer=self.bias_regu,
                kernel_initializer=self.kernel_init, 
                kernel_regularizer=self.kernel_regu)(gate)
        W_g_n = BatchNormalization()(W_g)

        add = Add()([W_x_n, W_g_n])
        add = Activation('relu')(add)

        psi = Conv1D(F_int, 1, strides=1, padding=self.padding,
                bias_regularizer=self.bias_regu,
                kernel_initializer=self.kernel_init, 
                kernel_regularizer=self.kernel_regu)(add)
    ...
    def conv_unit(self, inputs, nb_filter, stride_size, name=None):
        ...
        if self.max_pool:
            ...
            u = MaxPooling1D(pool_size=self.pool_size, padding=self.padding,
                    name=name)(u)
        else:
            if (stride_size != None) :
    ...
    def RRconv_unit(self, inputs, nb_filter, stride_size, name=None):
        ...
        if stride_size==None:
            u = self.conv_unit(inputs=inputs, 
                nb_filter=nb_filter, stride_size=None)
        else:
            u = self.conv_unit(inputs=inputs, 
                nb_filter=nb_filter, stride_size=stride_size)
        conv_1x1 = self.conv1d(nb_filter=nb_filter, stride_size=1)(u)
        for i in range(self.RRconv_time):
            if i == 0:
                r_u = u
            r_u = Add()([r_u, u])
            r_u = self.conv_unit(inputs=r_u, 
                nb_filter=nb_filter, stride_size=None)
        
        return Add(name=name)([r_u, conv_1x1])

    def upconv_unit(self, inputs, nb_filter, concatenate_layer, 
            apply_attention=False, att_transformer=False, name=None):
    ...
    def build_mtan_R2unet(self, pretrained_weights=None, 
        ...
        conv_init_exp = self.RRconv_unit(inputs=inputs,
            nb_filter=self.nb_filters[0], stride_size=None, name='E0')
        PS_mtan_init = self.mtan_att_block(pre_att_layer=conv_init_exp,
            pre_target=conv_init_exp, target=conv_init_exp,
            nb_filter_in=self.nb_filters[0], nb_filter_out=self.nb_filters[0],
            mode='down', strides=None, name='PS_mtan_E0')
        M_mtan_init = self.mtan_att_block(pre_att_layer=conv_init_exp,
            pre_target=conv_init_exp, target=conv_init_exp,
            nb_filter_in=self.nb_filters[0], nb_filter_out=self.nb_filters[0],
            mode='down', strides=None, name='M_mtan_E0') 
        ...
        for i in range(len(self.nb_filters)-1):
            ...
            if i == 0:
                exp_E = self.RRconv_unit(inputs=conv_init_exp, 
                    nb_filter=self.nb_filters[i],
                    stride_size=None, name=f'exp_E{i}') 
 
                PS_mtan_E = self.mtan_att_block(pre_att_layer=PS_mtan_init, 
                    pre_target=conv_init_exp, target=exp_E, 
                    nb_filter_in=self.nb_filters[i], 
                    nb_filter_out=self.nb_filters[i+1],
                    mode='down', strides=self.stride_size, 
                    name=f'PS_mtan_E{i}')
                M_mtan_E = self.mtan_att_block(pre_att_layer=M_mtan_init, 
                    pre_target=conv_init_exp, target=exp_E, 
                    nb_filter_in=self.nb_filters[i], 
                    nb_filter_out=self.nb_filters[i+1],
                    mode='down', strides=self.stride_size, 
                ...
                E = self.RRconv_unit(inputs=exp_E, 
                    nb_filter=self.nb_filters[i+1], 
                    stride_size=self.stride_size,
                    name=f'E{i+1}')
            else:
                exp_E = self.RRconv_unit(inputs=E, 
                    nb_filter=self.nb_filters[i], stride_size=None, 
                    name=f'exp_E{i}')  

                PS_mtan_E = self.mtan_att_block(pre_att_layer=PS_mtan_E, 
                    pre_target=E, target=exp_E, 
                    nb_filter_in=self.nb_filters[i], 
                    nb_filter_out=self.nb_filters[i+1],
                    mode='down', strides=self.stride_size, 
                    name=f'PS_mtan_E{i}') 
                M_mtan_E = self.mtan_att_block(pre_att_layer=M_mtan_E, 
                    pre_target=E, target=exp_E, 
                    nb_filter_in=self.nb_filters[i], 
                    nb_filter_out=self.nb_filters[i+1],
                    mode='down', strides=self.stride_size, 
                ...
                E = self.RRconv_unit(inputs=exp_E, 
                    nb_filter=self.nb_filters[i+1], 
                    stride_size=self.stride_size, name=f'E{i+1}') 
            ...
            if i == len(self.nb_filters)-2:
                exp_E = self.RRconv_unit(inputs=E, 
                    nb_filter=self.nb_filters[i+1], stride_size=None, 
                    name=f'exp_E{i+2}')
                exp_Es.append(exp_E)
        ...
        for i in range(len(self.nb_filters)):
            if i == 0:     
                D = self.upconv_unit(inputs=Es[-1], 
                    nb_filter=self.nb_filters[-1-i], 
                    concatenate_layer=Es[-1-i], apply_attention=False)
            else:
                D = self.upconv_unit(inputs=D_fus, 
                    nb_filter=self.nb_filters[-1-i], 
                    concatenate_layer=Es[-1-i], apply_attention=False) 
                            
            D_fus = self.RRconv_unit(inputs=D, 
                    nb_filter=self.nb_filters[-1-i], 
                    stride_size=None, name=f'D{i}')

            PS_mtan_D = self.mtan_att_block(pre_att_layer=PS_mtan_Es[-1-i], 
                    pre_target=D, target=D_fus,
                    nb_filter_in=self.nb_filters[-1-i], 
                    nb_filter_out=self.nb_filters[-1-i],
                    mode='up', strides=self.stride_size, name=f'PS_mtan_D{i}')
            M_mtan_D = self.mtan_att_block(pre_att_layer=M_mtan_Es[-1-i], 
                    pre_target=D, target=D_fus,
                    nb_filter_in=self.nb_filters[-1-i], 
                    nb_filter_out=self.nb_filters[-1-i],
                    mode='up', strides=self.stride_size, name=f'M_mtan_D{i}')    
            Ds.append(D_fus)
        ...
        outPS = Conv1D(3, 1,
            ...
            kernel_initializer=self.kernel_init, 
            kernel_regularizer=self.kernel_regu,
            name='pred_PS')(PS_mtan_D)
        outM = Conv1D(2, 1,
            bias_regularizer=self.bias_regu,
            kernel_initializer=self.kernel_init, 
            kernel_regularizer=self.kernel_regu,
            name='pred_mask')(M_mtan_D)
