...
def mosaic_basic_info(read_path_info, psres_multiple=1.5,
            base_wf_sec=60, buffer_secs_bef_P=0.5, dt=0.01):
    '''
    ...
    init_ent = np.array(p_utc) + \
                psres_multiple * np.array(residual_ps)
    ...
    if base_period >= base_wf_sec:
        ...
        return p_utc, s_utc, trc_pairs,\
                 init_stt, init_ent, base_period
                 
def mosaic_wf_marching(base_wf_sec,
         dt, base_period, data_npts, 
         p_utc, s_utc, trc_pairs, init_stt, init_ent,
         marching=True, marching_range=(1, 7),
         joint_snr_thre=3, suffix_Psnr_thre=1.5, scaling=False,
         EEW_march=False, buffer_EEWA_secs_bef_P=3):

    available_pts = np.round(
        data_npts - base_period/dt).astype('int')
    ...
    assign_res = np.random.multinomial(available_pts,
         np.random.dirichlet(np.ones(len(trc_pairs)+1)))*dt
    ## set up starttime and endtime point for each trace
    ## sum up to base_wf_sec
    slice_idx_center = []
    for ass in range(len(trc_pairs)):
        slice_idx_center.append(
            [init_stt[ass], init_ent[ass]+assign_res[ass+1]])
        if ass == 0:
        ...
        try:
            shift_for_sec, shift_back_sec = \
                np.random.randint(
                    ...
                    (p_s_residual[0]+assign_res[0])/dt)*dt, \
                np.random.randint(
                    (p_s_residual[-1]*0.5+assign_res[-1])/dt,
                    (p_s_residual[-1]*1.5+assign_res[-1]-0.5)/dt)*dt
        except:
    ...
    mosaic_utc = []
    slice_idx_to_end = []
    total_sec = base_wf_sec + shift_for_sec + shift_back_sec\
                 + buffer_for_mosaic     
    ...
    base_trc = sac_len_complement(
        trc_pairs[0].slice(
            slice_idx_to_end[0][0], slice_idx_to_end[0][1])
    )
    ...
    for ct, p in enumerate(slice_idx_to_end):
        if ct == 0:
            ...
            mosaic_trc = sac_len_complement(
                trc_pairs[ct].slice(p[0], p[1]))

            mosaic_pt = cumsum_npts[ct-1]

            new_p_utc.append( slice_idx_to_end[0][0] +\
                mosaic_pt*dt + (p_utc[ct] - p[0]) )
            new_s_utc.append(slice_idx_to_end[0][0] +\
                mosaic_pt*dt + (s_utc[ct] - p[0]) )    
            ...
            for m in range(len(mosaic_ENZ)):
                ...
                if mosaic_n >= avail_npts:
                    base_trc[m].data[
                        mosaic_pt:mosaic_pt+avail_npts] +=\
                        mosaic_ENZ[m][:avail_npts]*scale
                else:
                    base_trc[m].data[
                        mosaic_pt:mosaic_pt+mosaic_n] +=\
                        mosaic_ENZ[m][:mosaic_n]*scale
            new_mosaic_utc.append(
                slice_idx_to_end[0][0] + mosaic_pt*dt)
    ...
    for ms in range(len(mosaic_stt)):
        ...
        tp_to_ori_pt = [np.round(i/dt).astype(int) 
                                for i in tp_to_ori_sec]
        ts_to_ori_pt = [np.round(i/dt).astype(int) 
                                for i in ts_to_ori_sec]   
        joint_pt = [np.round(i/dt).astype(int) 
                                for i in joint_sec]   
    ...
    joint_ENZsnrs_pairs = MWA_joint_ENZsnr(trc_mosaic_pair[1],
         joint_pt_pair[1], snr_win=0.5, dt=0.01, 
         hpfreq=2, mode='std')
    # check P SNRs is large enough
    suffix_Psnrs = MWA_suffix_Psnr(trc_mosaic_pair[1],
        tp_to_ori_pt_pair[1], snr_win=3, dt=0.01,
         hpfreq=2, mode='std')
    ...
    if not np.array_equal((3, data_npts),
        np.unique([i.shape for i in trc_mosaic_pair])):
        return None, None, None
    ...
    if marching == True:
        return np.array(trc_mosaic_pair),\
                np.array(tp_to_ori_pt_pair),\
                np.array(ts_to_ori_pt_pair)
    else:
        return np.array(trc_mosaic_pair[1]),\
...
def mosaic_tar_func(data_npts, err_win_p, err_win_s, dt,
            tp_to_ori_pt, ts_to_ori_pt):
    ## gen_tar_func
    ...
    for n in range(len(tp_to_ori_pt)):
        tar_p_data += gen_tar_func(
            data_npts, tp_to_ori_pt[n], err_win_npts_p)
        tar_s_data += gen_tar_func(
            data_npts, ts_to_ori_pt[n], err_win_npts_s)
    tar_nz_data = np.ones(data_npts) - tar_p_data - tar_s_data
    label_psn = np.array([tar_p_data, tar_s_data, tar_nz_data]).T
    return label_psn


def mosaic_tar_func_detect(data_npts, err_win_p, err_win_s, dt,
            tp_to_ori_pt, ts_to_ori_pt, label='gaussian'):
    if label == 'gaussian':
    ...
    for n in range(len(tp_to_ori_pt)):
        tar_p_data += tar_function(
            data_npts, tp_to_ori_pt[n], err_win_npts_p)

    for m in range(len(ts_to_ori_pt)):
        tar_s_data += tar_function(
            data_npts, ts_to_ori_pt[m], err_win_npts_s)
    ...
    if len(tp_to_ori_pt) == len(ts_to_ori_pt) > 1:
        if np.min(ts_to_ori_pt) < np.min(tp_to_ori_pt):
            ...
            for arr_pt in range(len(tp_to_ori_pt)-1): 
                trc_mask[
                    int(tp_to_ori_pt[arr_pt]):\
                    int(ts_to_ori_pt[arr_pt+1])+1] = 1
        else:         
            for arr_pt in range(len(tp_to_ori_pt)): 
                trc_mask[
                    int(tp_to_ori_pt[arr_pt]):\
                    int(ts_to_ori_pt[arr_pt])+1] = 1
...
def mosaic_wf_plot(trc_3C, label_psn, new_tps, 
                    new_tss, save=None, show=False):
    wf_data = trc_3C.T
    tar_func = label_psn.T

    x_plot = [wf_data[0], wf_data[1], wf_data[2],
             tar_func[0], tar_func[1], tar_func[2]]
    ...
    for i in range(6):
        ...
        if 2 >= i: 
            for j in range(len(new_tps)):
                ax[i].axvline(x=new_tps[j],
                    label='Manual picked P', color='k', linewidth=1)
            for j in range(len(new_tss)):
                ax[i].axvline(x=new_tss[j], 
                    label='Manual picked S', color='r', linewidth=1)
...
def mosaic_wf_plot_detect(trc_3C, label_psn, mask, new_tps, 
                    new_tss, save=None, show=False):
    wf_data = trc_3C.T
    tar_func = label_psn.T
    mask_func = mask.T
    ...
    x_plot = [wf_data[0], wf_data[1], wf_data[2],
             tar_func[0], tar_func[1], mask_func[0]]
    label = ["E comp.", "N comp.", "Z comp.",
             "P prob.", "S prob.", 'Mask prob']
    fig, (ax1, ax2, ax3, ax4, ax5, ax6) = \
            plt.subplots(6, 1, sharex=True, figsize=(8, 8))
    ax=[ax1, ax2, ax3, ax4, ax5, ax6]
    for i in range(6):
