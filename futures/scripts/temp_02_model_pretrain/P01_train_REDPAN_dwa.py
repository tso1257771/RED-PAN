...
glob_list = np.random.permutation(
    glob("../temp_01_gen_sample_from_INSTANCE/tfrecord/*/*.tfrecord")
)
...
train_steps = int(
    np.floor(len(train_list) / (batch_size_per_replica * n_device) * train_epoch)
)
# dwa parameters
T = 2
...
with strategy.scope():
    ...
    for i in range(train_epoch):
        ...
        if i == 0 or i == 1:
            ...
            lambda_weights[0, i] = (
                2 * np.exp(w_pick / T) / (np.exp(w_pick / T) + np.exp(w_mask / T))
            )
            lambda_weights[1, i] = (
                2 * np.exp(w_mask / T) / (np.exp(w_pick / T) + np.exp(w_mask / T))
            )
        label_mask_weight = lambda_weights[:, i]
        logging.info(
            f"Pick weighting:{label_mask_weight[0]:.2f}\t"
        ...
        dis_train = iter(
            strategy.experimental_distribute_dataset(
                tfrecord_dataset_detect(
                    train_list, data_length=data_length, batch_size=global_batch_size
                )
            )
        )
        dis_val = iter(
            strategy.experimental_distribute_dataset(
                tfrecord_dataset_detect(
                    val_list, data_length=data_length, batch_size=global_batch_size
                )
            )
        )
        ...
        for train_step in range(train_steps_per_epoch):
            ...
            train_loss = (
                tf.reduce_mean(mean_train_batch_loss) / strategy.num_replicas_in_sync
            )
            train_loss_label = (
                tf.reduce_mean(mean_batch_loss_label) / strategy.num_replicas_in_sync
            )
            train_loss_mask = (
                tf.reduce_mean(mean_batch_loss_mask) / strategy.num_replicas_in_sync
            )
            ...
            progbar = tf.keras.utils.Progbar(
                train_steps_per_epoch, interval=0.1, stateful_metrics=["step"]
            )
        ...
        for val_step in range(val_steps_per_epoch):
            ...
            dis_val_trc_in, dis_val_label_in, dis_val_mask_in, dis_val_idx = next(
                dis_val
            )
            ...
            val_loss = (
                tf.reduce_mean(mean_val_batch_loss) / strategy.num_replicas_in_sync
            )
        ...
        progbar.update(
            train_step + 1,
            values=(("train_loss", train_loss_f), ("val_loss", val_loss_f)),
        )
        ...
        if track_item < track_loss:
            message = save_model(model, save_dir, train_loss_epoch, val_loss_epoch)
        ...
        if ct_not_improve == not_improve_patience:
            logging.info(
                "Performance has not improved for "
