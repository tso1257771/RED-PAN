...
plt_fig = True
pred_interval_sec = 0.2
dt = 0.01
pred_npts = 6000
# only trigger P arrivals in the last 1 second of data with 100Hz sampling rate
real_time_p_trg_search = [5700, 6000]
p_snr_thre = 3
p_maintain_thre = 1 # threshold for P picks maintenance (sec)
...
fig_path = f"./{outtrg_path}/fig_trg_HL_{sta}_{mdl_hdr}"
os.system(f"rm -rf {fig_path}")
if not os.path.exists(fig_path):
    os.makedirs(fig_path)
trg_out = os.path.join(outtrg_path, f"trg_HL_{sta}.txt")
...
wf = read(f"./HL_data/*.CHKH.*")
info = wf[0].stats

_init_stt = wf[0].stats.starttime
...
# temporary triggered information (with consider P picks maintenance time0)
queued_trg_pool = []
queued_trg_pool_confidence = []
queued_trg_pool_maintain_sec = []
queued_trg_utc_pool = []
queued_trg_frame_utc_pool = []

for i in range(n_slices):
    logging.info(f"Processing segments: {i+1}/{n_slices}")
    if init_pt + pred_npts > len(wf[0].data):
        continue
    current_stt = _init_stt + i*pred_interval_sec
    ...
    wf_data = np.stack([wf_E, wf_N, wf_Z], -1
        )[np.newaxis, ...].astype("float32")

    model.set_tensor(input_details[0]["index"], wf_data)
    model.invoke()
    ...
    search_pt_st = real_time_p_trg_search[0] 
    search_pt_end = real_time_p_trg_search[1]
    search_npts = real_time_p_trg_search[1] - real_time_p_trg_search[0]
    # search potential real-time P picks within receptive field
    p_peaks, p_peaks_info = find_peaks(rt_p[search_pt_st:search_pt_end], 
        height=0.3, distance=int(1 / dt))
    p_peaks += search_pt_st
    p_peaks_confidence = p_peaks_info["peak_heights"]

    # set threshold for the real-time P peaks
    bool_p_peaks = []
    for _p in p_peaks:
        # reject if both P&S pick occur at same position
        check_no_S_trg = np.any(
            rt_s[int(_p - 0.2 / dt) : int(int(_p + 0.2 / dt))] > 0.3
        )

        if check_no_S_trg == False:
            bool_p_peaks.append(True)
        else:
            bool_p_peaks.append(False)

    bool_p_peaks = np.array(bool_p_peaks)

    if len(bool_p_peaks) > 0:
        p_peaks = p_peaks[bool_p_peaks]
        p_rt_pb = p_peaks_confidence[bool_p_peaks]
    ...
    if len(p_peaks) >= 1:
        ...
        for t in range(len(p_peaks)):
            check_same_pick_pt_err = np.abs(
                np.array(queued_trg_pool) - (init_pt+p_peaks[t]) 
                ) 

            if not np.any(check_same_pick_pt_err < 1/dt):
                queued_trg_pool.append( init_pt+p_peaks[t] )
                queued_trg_pool_confidence.append(p_rt_pb[t])
                queued_trg_frame_utc_pool.append( current_stt + pred_npts*dt )
                queued_trg_pool_maintain_sec.append(pred_interval_sec)
                #stop
            else:
                # consider the same pick with 1 seconds error
                potential_same_pick_idx = np.where(
                    check_same_pick_pt_err < 1/dt)[0]
                
                if len(potential_same_pick_idx) == 0:
                    continue

                same_pick_idx = np.argmin(check_same_pick_pt_err)

                queued_trg_pool_maintain_sec[same_pick_idx] += pred_interval_sec
                print(f"Point {queued_trg_pool[same_pick_idx]}"+\
                    f" maintain trigger for "+\
                    f"{queued_trg_pool_maintain_sec[same_pick_idx]}"+\
                    f" seconds")

                # trigger the P arrivals if the pick maintain for seconds  
                if queued_trg_pool_maintain_sec[same_pick_idx] >= p_maintain_thre:
                    if not queued_trg_pool[same_pick_idx] in trg_pool:
                        trg_pool.append(queued_trg_pool[same_pick_idx])
                        trg_pool_confidence.append(
                            queued_trg_pool_confidence[same_pick_idx])
                        trg_utc_pool.append(
                            _init_stt + queued_trg_pool[same_pick_idx]*dt
                        )
                        trg_frame_utc_pool.append(
                            queued_trg_frame_utc_pool[same_pick_idx])
                        #stop
                            
            ### check if triggers are valid to store in queued trigger pools
            # 1. Check P snr
            check_p_snr = p_snrs[t] > p_snr_thre
            # 2. Check similar triggers issued within 1 seconds error
            check_issued = len(
                    np.where(
                        np.abs((p_peaks[t] + i*pred_interval_sec/dt )-\
                                np.array(trg_pool)
                        ) < 100
                    )[0]
                ) == 0
